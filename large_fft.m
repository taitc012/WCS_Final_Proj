function [abs_freq_result, abs_overlap_freq_result, noDC_deg, noDC_prob] = large_fft( recv, fft_len, N_fft, N_forward, N_forward_times, dirName );
%manual setting
...
%N_forward_times = 9040;    %WiSee 0.5sec/0.5ms
    ...
    freq = zeros( large_fft_size, 1 ); %record freq-domain data after large fft
    ...
    for jj=1:N_forward_times
        for ant_i=1:ANT_CNT
            ...
            if( samp_end>NUM_SAMP )
                disp( ['Sampleing ranges exceed the length of vector. The' int2str(jj) 'th Large-FFT'] );
                %break;
            end
            ...
            % large fft
            if(gpuDeviceCount>0)
                g_freq = fft( g_recv( samp_start:samp_end, ant_i ) );
                freq = gather( g_freq );
            else
                freq = fft( recv( samp_start:samp_end, ant_i ) );
            end
            ...
            %{
            %time_noDC(:,ant_i) = ifft( freq, large_fft_size );
            [noUSE, dc_index( jj, ant_i )] = max( abs( freq ) );
            result_noDC = abs( freq );
            %}
            ...
            if jj==1
                ...
                dc_index( jj, ant_i ) = dc_index( 1, ant_i );
            end
            %}
        ...
        %{
        time_noDC(:,1:7) = recv(samp_start:samp_end, 1:7);
        %}
        %dc_index(jj,:)'
        [ noDC_deg(:, jj), noDC_prob(:, jj) ] = music( time_noDC(:, 1:7)', n_signal, ANT_CNT-1, lambda, lambda/2, n_samples, 0 );
